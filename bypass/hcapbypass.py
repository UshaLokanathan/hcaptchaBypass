...
def REQ_Data(host, sitekey):
        try:
            r = http.get(f"https://hcaptcha.com/checksiteconfig?host={host}&sitekey={sitekey}&sc=1&swa=1", headers=headers, timeout=4)
            if r.json()["pass"]:
                return r.json()["c"]
            else:
                return False
...
def Get_Captcha(host, sitekey, n, req):
        try:
            ...
            r = http.post(f"https://hcaptcha.com/getcaptcha?s={sitekey}",cookies=cookies, data=data, headers=headers, timeout=4)
            return r.json()
        except Exception as e:
            print(e)
            return False
